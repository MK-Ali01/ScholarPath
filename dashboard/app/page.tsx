    ...
    <main className="page">
      <header className="masthead">
        <span className="eyebrow">
          ScholarPath — Pipeline Log
        </span>

        <h1>Candidate Runs</h1>

        <p className="sub">
          Phase 1 — Intake, Profile Extraction, Domain Recommendation
        </p>
      </header>
                  ...
                  <div className="scholarships">
                    ...
                    {[...p.scholarship_matches]
                      .sort((a, b) => {
                        ...
                        return (
                          order[a.match_confidence] -
                          order[b.match_confidence]
                        );
                      })
                      .map((m) => (
                        <div
                        ...
                        >
                          <div className="scholarship-head">
                            <a
                              href={m.scholarships.source_url}
                              target="_blank"
                              rel="noreferrer"
                              className="scholarship-name"
                            >
                              {m.scholarships.name}
                            </a>

                            <span
                              className={`confidence confidence-${m.match_confidence}`}
                            >
                              {m.match_confidence}
                            </span>
                          </div>

                          <p className="scholarship-reasoning">
                            {m.match_reasoning}
                          </p>

                          <div className="scholarship-meta">
                            <span>
                              {m.scholarships.country_or_region}
                            </span>
                            <span>·</span>
                            <span>
                              {m.scholarships.deadline_text}
                            </span>
                          </div>

                          {m.country_eligibility_status ===
                            'not_verified' && (
                            <p className="unverified-flag">
                              ⚠ Country eligibility not verified —
                              confirm manually before relying on this
                              match
                            </p>
                          )}

                          <span className="verified-date">
                            Scholarship data last verified{' '}
                            {new Date(
                              m.scholarships.last_verified_on
                            ).toLocaleDateString()}
                          </span>
                        </div>
                      ))}
